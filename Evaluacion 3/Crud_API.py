import requests
import json
from Autentificacion import Autenticacion
...
class APIClient:
    ...
    def get_data_ID(self, endpoint):
        print("\n--- Ver dato por ID ---")
        base_url = self.base_url  # Base URL definida en la clase.
        headers = {"Authorization": f"Bearer {self.api_key}"}  # Cabecera con la API key.

        while True:  # Ciclo para solicitar la ID hasta que sea válida.
            try:
                # Solicitar la ID al usuario.
                item_id = input("Ingrese la ID del dato que desea ver: ").strip()
                # Construir la URL con la ID.
                ...
                if response.status_code == 200:  # Si la respuesta es exitosa.
                    item = response.json()  # Convertir la respuesta a JSON.
                    print("Dato obtenido con GET:")
                    print(json.dumps(item, indent=4))  # Mostrar el dato con formato.
                    return item  # Retornar el objeto encontrado.
                
                elif response.status_code == 404:  # Si no se encuentra la ID.
                    print("ID no encontrada. Por favor, ingrese una ID válida.")
                else:  # Otros errores.
    ...
    def get_data_ID_Values(self, endpoint):
        ...
        while True:  # Ciclo para solicitar la ID hasta que sea válida.
            try:
                # Solicitar la ID al usuario.
                item_id = input("Ingrese la ID del dato que desea sincronizar: ").strip()
                # Construir la URL con la ID.
                url = f"{base_url}/{endpoint}/{item_id}"
                ...
                if response.status_code == 200:  # Si la respuesta es exitosa.
                    ...
                    print("Dato obtenido con GET:")
                    print(json.dumps(item, indent=4))  # Mostrar el dato con formato.
                    values=list(item.values())  # Retornar el objeto encontrado.
                    return values
...
def menu_API():
    ...
    while True:
        ...
        if opcion == "1": # Opcion que usa el metodo GET.
            print("\n--- Como desea filtrar los datos a mostrar ---")
            print("1. Selecionar por ID")
            print("2. Selecionar una cantidad")
            opcion_filtrado = input("Seleccione una opción (1 o 2): ")
            if opcion_filtrado == "1":
                client.get_data_ID(endpoint_api) # Usando el endpoint se hace uso del metodo GET.
            elif opcion_filtrado == "2":
                client.get_data(endpoint_api)
            else:
